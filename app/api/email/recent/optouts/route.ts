...
// GET endpoint to fetch recent email opt-outs
export async function GET(request: NextRequest) {
  try {
    ...
    // Fetch recent opt-outs
    const emailManager = new EmailManager();
    const { data, error } = await emailManager.getRecentOptOuts(limit, offset);

    if (error) {
      console.error('Error fetching recent email opt-outs:', error);
      return NextResponse.json(
        { success: false, error: 'Error fetching recent email opt-outs' },
        { status: 500 }
      );
    }
    ...
    console.error('Unexpected error in GET /api/email/recent/optouts:', error);
    return NextResponse.json(
